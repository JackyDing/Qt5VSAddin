...
void AppendVisualizer(LPWSTR file, LPWSTR pwzQt5Visualizer)
{
    AutoExp autoExp(file);
    autoExp.RemoveSection(L"QT_DEBUG_START", L"QT_DEBUG_END");
    autoExp.AppendSection(L"Visualizer", Tokenize(pwzQt5Visualizer));
}
...
UINT __stdcall AppendQt5Visualizer(MSIHANDLE hInstall)
{
	HRESULT hr = S_OK;
	UINT er = ERROR_SUCCESS;
    INSTALLSTATE isQt5VSAddin2010Action;
    INSTALLSTATE isQt5VSAddin2010Installed;
    INSTALLSTATE isQt5VSAddin2012Action;
    INSTALLSTATE isQt5VSAddin2012Installed;
    INSTALLSTATE isQt5VSAddin2013Action;
    INSTALLSTATE isQt5VSAddin2013Installed;

    LPWSTR pwzVs2010AutoExpDat = NULL;
    LPWSTR pwzVs2012AutoExpDat = NULL;
    LPWSTR pwzVs2013AutoExpDat = NULL;

    LPWSTR pwzQt5Visualizer2010 = NULL;
    LPWSTR pwzQt5Visualizer2012 = NULL;
    LPWSTR pwzQt5Visualizer2013 = NULL;

	hr = WcaInitialize(hInstall, "AppendQt5Visualizer");
	ExitOnFailure(hr, "Failed to initialize");

	WcaLog(LOGMSG_STANDARD, "Initialized.");

    ::MsiGetFeatureState(hInstall, L"Qt5VSAddin2010", &isQt5VSAddin2010Installed, &isQt5VSAddin2010Action);
    ::MsiGetFeatureState(hInstall, L"Qt5VSAddin2012", &isQt5VSAddin2012Installed, &isQt5VSAddin2012Action);
    ::MsiGetFeatureState(hInstall, L"Qt5VSAddin2013", &isQt5VSAddin2013Installed, &isQt5VSAddin2013Action);
    
    if (isQt5VSAddin2010Action == 3)
    {
        hr = WcaGetProperty(L"VS2010_AUTOEXPDAT", &pwzVs2010AutoExpDat);
        ExitOnFailure(hr, "failed to get install location");
        ExtractBinary(L"Qt5Visualizer2010", &pwzQt5Visualizer2010);
        ExitOnFailure(hr, "failed to get Qt5Visualizer2010");
        AppendVisualizer(pwzVs2010AutoExpDat, pwzQt5Visualizer2010);
    }

    if (isQt5VSAddin2012Action == 3)
    {
        hr = WcaGetProperty(L"VS2012_AUTOEXPDAT", &pwzVs2012AutoExpDat);
        ExitOnFailure(hr, "failed to get install location");
        ExtractBinary(L"Qt5Visualizer2012", &pwzQt5Visualizer2012);
        ExitOnFailure(hr, "failed to get Qt5Visualizer2012");
        AppendVisualizer(pwzVs2012AutoExpDat, pwzQt5Visualizer2012);
    }

    if (isQt5VSAddin2013Action == 3)
    {
        hr = WcaGetProperty(L"VS2013_AUTOEXPDAT", &pwzVs2013AutoExpDat);
        ExitOnFailure(hr, "failed to get install location");
        ExtractBinary(L"Qt5Visualizer2013", &pwzQt5Visualizer2013);
        ExitOnFailure(hr, "failed to get Qt5Visualizer2013");
        AppendVisualizer(pwzVs2013AutoExpDat, pwzQt5Visualizer2013);
    }

LExit:
    ReleaseStr(pwzQt5Visualizer2010);
    ReleaseStr(pwzQt5Visualizer2012);
    ReleaseStr(pwzQt5Visualizer2013);
...
UINT __stdcall RemoveQt5Visualizer(MSIHANDLE hInstall)
{
	HRESULT hr = S_OK;
	UINT er = ERROR_SUCCESS;
	...
	hr = WcaInitialize(hInstall, "RemoveQt5Visualizer");
	ExitOnFailure(hr, "Failed to initialize");
    ...
    if (isQt5VSAddin2010Installed == 3)
    {
        hr = WcaGetProperty(L"VS2010_AUTOEXPDAT", &pwzVs2010AutoExpDat);
        ExitOnFailure(hr, "failed to get install location");
        RemoveVisualizer(pwzVs2010AutoExpDat);
    }
	if (isQt5VSAddin2012Installed == 3)
    {
        hr = WcaGetProperty(L"VS2012_AUTOEXPDAT", &pwzVs2012AutoExpDat);
        ExitOnFailure(hr, "failed to get install location");
        RemoveVisualizer(pwzVs2012AutoExpDat);
    }
    if (isQt5VSAddin2013Installed == 3)
    {
        hr = WcaGetProperty(L"VS2013_AUTOEXPDAT", &pwzVs2013AutoExpDat);
        ExitOnFailure(hr, "failed to get install location");
